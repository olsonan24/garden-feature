...
export default function Jarvis() {
  ...
  return <div className="shell">
    ...
    <main><header><button className="menu" aria-label="Open navigation" onClick={() => setMobile(true)}><Menu /></button><label><Building2 /><select aria-label="Account" value={accountId} onChange={(event) => changeAccount(event.target.value)}>{accounts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><ChevronDown /></label><label><CalendarDays /><select aria-label="Saved reporting week" value={selectedPeriod.id.startsWith("empty-") ? "" : selectedPeriod.id} onChange={(event) => changePeriod(event.target.value)}><option value="" disabled>{accountPeriods.length ? "Select a saved week" : "No saved weeks"}</option>{accountPeriods.map((period) => <option key={period.id} value={period.id}>{period.label}</option>)}</select><ChevronDown /></label><button className="primary upload" onClick={() => setModal("upload")}><Upload /> Upload Reports</button></header>
      ...
      {view === "overview" && <section><Title eyebrow="Weekly command brief" title="Portfolio Overview" sub={`${selectedPeriod.label} · ${matchingPortfolioPeriods.length || 1} account${(matchingPortfolioPeriods.length || 1) === 1 ? "" : "s"}`} right={<Pill status={portfolioStatus(portfolioPeriods)} />} />
      ...
      {view === "accounts" && <section><Title eyebrow="Account command center" title={account.name} sub={`${accountSkus.length} SKUs · ${selectedPeriod.label}`} right={<div className="buttons"><button className="secondary" onClick={() => setModal("editAccount")}><Pencil /> Edit Name</button><button className="secondary" onClick={() => setModal("sku")}><PackagePlus /> Add SKU</button><button className="primary" onClick={() => go("review")}><ClipboardCheck /> Start Weekly Business Review</button></div>} />
        ...
        <div className="tabs">{accountSkus.map((sku) => <button key={sku.id} className={selectedSku?.id === sku.id ? "active" : ""} onClick={() => setSkuId(sku.id)}><i className={sku.status} />{sku.name}</button>)}</div>
        {selectedSku ? <div className="sku-grid"><Panel className="sku-main" title={selectedSku.name} sub={`${selectedSku.sku} · ${selectedSku.asin || "ASIN not mapped"}`} right={<Pill status={selectedSku.status} />}><div className="sku-metrics">{[["Net revenue", money(selectedSku.netSales)], ["Net proceeds", money(selectedSku.profit)], ["Sessions", integer(selectedSku.sessions)], ["Conversion", pct(selectedSku.conversion)], ["Ad spend", money(selectedSku.adSpend)], ["Ad sales", money(selectedSku.adSales)], ["ACoS", pct(selectedSku.adSales ? selectedSku.adSpend / selectedSku.adSales * 100 : selectedSku.adSpend ? 999 : 0)], ["Ad orders", integer(selectedSku.adOrders)]].map(([label, value]) => <div key={label}><span>{label}</span><b className={label === "Net proceeds" && selectedSku.profit < 0 ? "bad" : ""}>{value}</b></div>)}</div></Panel><Panel title="Inventory Snapshot" sub={`Latest package · ${formatDate(selectedPeriod.generatedAt)}`}><div className="inventory"><b>{integer(selectedSku.inventory)}</b><span>Total FBA</span><div>{[["Fulfillable", selectedSku.fulfillable], ["Reserved", selectedSku.reserved], ["FC transfer", selectedSku.transfer]].map(([label, value]) => <p key={label}><small>{label}</small><strong>{integer(Number(value))}</strong></p>)}</div></div></Panel><ListingReputation sku={selectedSku} onSaved={(updated) => setSkus((current) => unique([...current.filter((item) => item.id !== updated.id), updated]))} onToast={setToast} /><SkuEconomicsPanel key={selectedSku.id} sku={selectedSku} onSave={saveSkuEconomics} /><Panel title="What Happened" className="danger"><p className="copy">{selectedSku.issue}</p></Panel><Panel title="Recommended Change" className="recommend"><p className="copy">{selectedSku.recommendation}</p><button className="link" onClick={() => setModal("action")}>Add to checklist <ChevronRight /></button></Panel></div> : <Panel title="No SKU data" sub="Add a SKU manually or upload the weekly report package."><button className="primary" onClick={() => setModal("sku")}><PackagePlus /> Add SKU</button></Panel>}
        ...
        <Panel title="SKU Directory" sub={`All products attached to ${account.name}`} right={<label className="search"><Search /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search SKU or ASIN" /></label>}><Table><thead><tr><th>Product</th><th>SKU / ASIN</th><th>Net Revenue</th><th>Sessions</th><th>Units</th><th>Conversion</th><th>Inventory</th><th>Net Proceeds</th><th>Status</th></tr></thead><tbody>{accountSkus.filter((sku) => `${sku.name}${sku.sku}${sku.asin}`.toLowerCase().includes(search.toLowerCase())).map((sku) => <tr key={sku.id} onClick={() => setSkuId(sku.id)}><td><b>{sku.name}</b></td><td>{sku.sku}<small>{sku.asin}</small></td><td><b>{money(sku.netSales)}</b></td><td>{integer(sku.sessions)}</td><td>{integer(sku.units)}</td><td>{pct(sku.conversion)}</td><td>{integer(sku.inventory)}</td><td className={sku.profit < 0 ? "bad" : ""}>{money(sku.profit)}</td><td><Pill status={sku.status} /></td></tr>)}</tbody></Table></Panel>
...
function Pill({ status }: { status: Status }) { return <span className={`pill ${status}`}><AlertTriangle size={12} />{status === "attention" ? "Needs attention" : status}</span>; }
...
function portfolioStatus(periods: PeriodPayload[]): Status { return periods.some((period) => period.status === "critical") ? "critical" : periods.some((period) => period.status === "attention") ? "attention" : periods.some((period) => period.status === "monitor") ? "monitor" : "healthy"; }
