...
import { auditListingDiagnostics, listingDiagnosticsReadiness } from "./amazon/listing-diagnostics";
...
export async function runDeepAccountAudit(db: PersistentDatabase, principal: JarvisPrincipal, accountId: string) {
  ...
  const listing = auditListingDiagnostics(input, id);
  const findings: AuditFinding[] = [...triage.findings, ...sku.findings, ...ppc.findings, ...listing];
  const readiness = combinedReadiness([triage.dataReadiness, sku.readiness, ppc.readiness, brand.readiness, listingDiagnosticsReadiness()]);
